...
public class DownloadManager 
{
	...
	static public DownloadManager getInstance(Context context) 
	{
		if (mInstance == null) 
		{
			synchronized (DownloadManager.class) 
			{
				if (mInstance == null) 
				{
					LogUtil.i(LogUtil.TAG, "+++ new DownloadManager +++");
	...
	public void addDownloadTask(String url) 
	{
		if (isDownloadTaskTaskExist(url))
		{
			if (mPauseList.contains(url))
			{
				 ...
				 * 如果有，则创建一个新的任务
				 */
				LogUtil.i(LogUtil.TAG, "++++ resume downloadtask ++++");
			...
			else
			{
				 ...
				 */
				LogUtil.i(LogUtil.TAG, "++++ go on downloadtask ++++");
				return;
			}
		}
		else
		{
			//添加下载任务
			LogUtil.i(LogUtil.TAG, "++++ start new task ++++");
	...
	public void pauseDownloadTask(String url) 
	{
		for (DownloadTask item : mDownloaderList) 
		{
			...
			{
				LogUtil.i(LogUtil.TAG," ++++ pauseDownloadTask ++++");
				mPauseList.add(url);
				item.pauseTask();
				break;
			}
	...
	public void resumeDownloadTask(String url) 
	{
		for (DownloadTask item : mDownloaderList) 
		{
			if (item.getDownloadUrl().equalsIgnoreCase(url)) 
			{
				LogUtil.i(LogUtil.TAG," ++++ resumeDownloadTask ++++");
	...
	public void deleteDownloadTask(String url) 
	{
		for (DownloadTask item : mDownloaderList) 
		{
			if (item.getDownloadUrl().equalsIgnoreCase(url)) 
			{
				LogUtil.i(LogUtil.TAG," ++++ deleteDownloadTask ++++");
				item.deleteTask();
